...
#include <errno.h>
#include <string.h>
#include <sys/types.h>  //to use some C macros constants
#include <sys/socket.h> //socketpair(2) prototype
...
int main(int argc, char **argv)
{
	...
	if ( z == -1) {
		fprintf(stderr, "%s: socketpair(AF_LOCAL, SOCKET_STREAM,""0)\n", strerror(errno));
		return 1; /* Failed */
	}

	/* Write message to socket s[1]: */
	z = write( s[1], cp = "Hello?", 6);
	if (z < 0 ) {
		fprintf(stderr, "%s: write(%d,\"%s\",%d)\n", strerror(errno), s[1], cp, strlen(cp));
		return 2; /* Failed to write*/
	}
